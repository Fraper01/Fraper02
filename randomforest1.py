...
# Fase 4 Escalado de Caracteristicas (El escalador se usará también para el nuevo solicitante)
...
# Fase 6: Entrenamiento del Modelo Random Forest
rf_model = RandomForestClassifier(random_state=42)
rf_model.fit(X_train, y_train)
print("\n--- Modelo Random Forest Entrenado ---")

# Fase 7: Evaluación del Modelo Random Forest
cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
y_pred_rf = rf_model.predict(X_test)
print("Reporte de Clasificación en el conjunto de prueba (Random Forest):\n", classification_report(y_test, y_pred_rf))

f1_scores_rf_cv = cross_val_score(rf_model, X, y, cv=cv, scoring='f1_weighted')
print(f"F1-score promedio en validación cruzada (Random Forest): {np.mean(f1_scores_rf_cv):.4f}")
...
# Fase 8: Simulación de un nuevo solicitante
nuevo_solicitante = {
...
# Fase 9: Predicción para el nuevo solicitante con Random Forest
prediccion_rf_nuevo = rf_model.predict(nuevo_solicitante_final)
print("\n--- Análisis del Nuevo Solicitante (Random Forest) ---")
print(f"Predicción del nivel de riesgo para el nuevo solicitante: {prediccion_rf_nuevo}")
...
print(f"\nImportancia de las características (según el modelo):\n", feature_importances_rf.sort_values(ascending=False).head()) # Mostramos las 5 más importantes
print(f"\nPrimeras filas del conjunto de entrenamiento escalado:\n", X_train_scaled.head())

# Fase 10: Guardado del modelo Random Forest y el scaler
joblib.dump(rf_model, 'ProyectoIAGrupo4Rfmodelo.joblib')
print("\nModelo Random Forest guardado como ProyectoIAGrupo4Rfmodelo.joblib")

# Guardar el objeto StandardScaler ajustado
joblib.dump(scaler, 'ProyectoIAGrupo4Rfscaler.joblib')
print("Scaler guardado como ProyectoIAGrupo4Rfscaler.joblib")

# Guardar los nombres de las columnas de X_train
joblib.dump(X_train.columns.tolist(), 'ProyectoIAGrupo4Rfcolumns.joblib')
print("Nombres de las características guardados como ProyectoIAGrupo4Rfcolumns.joblib")

# Guardar los nombres de las columnas numéricas
joblib.dump(numeric_cols, 'ProyectoIAGrupo4Rf_numeric_cols.joblib')
print("Nombres de las columnas numéricas guardados como ProyectoIAGrupo4Rf_numeric_cols.joblib")

# Guardar las etiquetas del conjunto de entrenamiento
joblib.dump(y_train, 'ProyectoIAGrupo4Rf_y_train.joblib')
print("Etiquetas del conjunto de entrenamiento guardadas como ProyectoIAGrupo4Rf_y_train.joblib")

# Guardar el conjunto de entrenamiento escalado
joblib.dump(X_train_scaled, 'ProyectoIAGrupo4Rf_X_train_scaled.joblib')
print("Conjunto de entrenamiento escalado guardado como ProyectoIAGrupo4Rf_X_train_scaled.joblib")
