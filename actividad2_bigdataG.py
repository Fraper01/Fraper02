...
def visualizar_resultados(df_final):
    """
    Realiza visualizaciones de los datos de clientes limpios y organizados.

    Args:
        df_final (pandas.DataFrame): El DataFrame de clientes limpio y agrupado.
    """
    print("\n--- Generando Visualizaciones de los Resultados ---")

    if df_final is None or df_final.empty:
        print("No hay datos para visualizar. El DataFrame final está vacío o es None.")
        return
    ...
    # --- Gráfico 1: Destinos Favoritos más Populares ---
    plt.subplot(1, 2, 1) 
    top_destinos = df_final['Destino_favorito'].value_counts().head(7) 
    sns.barplot(x=top_destinos.index, y=top_destinos.values, hue=top_destinos.index, palette='viridis', legend=False)
    plt.title('Top 6 Destinos Favoritos de los Clientes', fontsize=14)
    plt.xlabel('Destino', fontsize=12)
    plt.ylabel('Número de Clientes', fontsize=12)
    plt.xticks(rotation=45, ha='right') 
    plt.tight_layout() 

    # --- Gráfico 2: Distribución del Gasto Total por Cliente 
    plt.subplot(1, 2, 2) 
    sns.histplot(df_final['Gasto_total_por_cliente'], bins=10, kde=True, color='skyblue')
    plt.title('Distribución del Gasto Total por Cliente', fontsize=14)
    ...
    sns.scatterplot(
        data=df_final,
        x='Numero_de_viajes',
        y='Gasto_total_por_cliente',
        hue='Destino_favorito', 
        size='Edad',            
        sizes=(20, 400),        
        alpha=0.7               
    )
    plt.title('Gasto Total vs. Número de Viajes por Cliente (coloreado por Destino)', fontsize=16)
    plt.xlabel('Número de Viajes', fontsize=12)
    plt.ylabel('Gasto Total por Cliente (€)', fontsize=12)
    plt.legend(title='Destino Favorito', bbox_to_anchor=(1.05, 1), loc='upper left') 
...
if __name__ == "__main__":
    ...
    if df_final_limpio_y_organizado is not None:
        ...
        visualizar_resultados(df_final_limpio_y_organizado)
